...
class SupportFnsCar:
    ...
    def __init__(self):
        ...
        # Constants from state space - for calculating states
        m=1500
        Iz=3000
        Caf=19000
        Car=33000
        lf=2
        lr=3
        Ts=0.02 # time interval or a cycle duration
        x_dot=20

        # constants for MPC
        hz=20 # horizon period
        outputs=2 # number of outputs: Y, psi
        ...
        # constants for reference signal (trajectory generation)
        lane_width=7 # [m]
        r=4 # amplitude for sinusoidal functions
        f=0.01 # frequency
        time_length = 10 # [s] - duration of the entire manoeuvre

        trajectory=1 # 3 different reference trajectories - choose 1,2,3
    ...
    def trajectory_generator(self,t,r,f):
        '''This method creates the reference trajectory for a car to follow
        Args:
            t (float array): time step
            r (int): amplitude for sine wave
            f (float): frequency [Hz]
        
        Returns:
            reference values (arrays) for psi, X, Y
        '''
        ...
        dx=x[1:len(x)]-x[0:len(x)-1] # [x[1]-x[0], x[2]-x[3], ...]
        dy=y[1:len(y)]-y[0:len(y)-1]

        # Define the reference yaw angles
        psi=np.zeros(len(x))
        psiInt=psi
        psi[0]=np.arctan2(dy[0],dx[0]) # angle from opposite and adjacent sides using tan
        psi[1:len(psi)]=np.arctan2(dy[0:len(dy)],dx[0:len(dx)])

        # We want the yaw angle to keep track the amount of rotations
        dpsi=psi[1:len(psi)]-psi[0:len(psi)-1] #rotation at each time step
        psiInt[0]=psi[0]
    ...
    def state_space(self):
        ...
        Returns:
            discrete system matrix Ad, discrete input matrix Bd, Cd, Dd'''
        ...
        A3=-(2*Caf*lf-2*Car*lr)/Iz*x_dot
        A4=-(2*Caf*lf**2+2*Car*lr**2)/Iz*x_dot

        B1= 2*Caf/m
        B2=2*Caf*lf/Iz

        A=np.array([[0, 1, x_dot, 0],[0, A1, 0, A2],[0,0,0,1],[0,A3,0,A4]])
        B=np.array([[0],[2*Caf/m],[0],[2*Caf*lf/Iz]])
    ...
    def mpc_simplification(self, Ad, Bd, Cd, Dd, hz):
        ...
        # Weight matrices for Hdb, Fdbt
        Q=self.constants['Q']
        S=self.constants['S']
        R=self.constants['R']

        # constructing Qdb for Hdb
    ...
    def open_loop_new_states(self,states,input1):
        ...
        for i in range(0,sub_loop):
            ...
            y_dot_dot=-(2*Caf+2*Car)/(m*x_dot)*y_dot+(-x_dot-(2*Caf*lf-2*Car*lr)/(m*x_dot))*psi_dot+2*Caf/m*input1
            psi_dot=psi_dot
            psi_dot_dot=-(2*lf*Caf-2*lr*Car)/(Iz*x_dot)*y_dot-(2*lf**2*Caf+2*lr**2*Car)/(Iz*x_dot)*psi_dot+2*lf*Caf/Iz*input1
